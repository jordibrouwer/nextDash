...
class ConfigManager {
    ...
    async setupEventListeners() {
        ...
        if (bulkToggleStatusBtn) {
            bulkToggleStatusBtn.addEventListener('click', () => {
                this.bookmarks.bulkToggleStatus(this.bookmarksData);
                this.refreshBookmarksList({ skipFlush: true });
                this.markDirty();
            });
        }
    ...
    setupGeneralCardCollapsible() {
        ...
        const defaultExpandedIds = new Set(['basics', 'dashboard']);
        ...
        cards.forEach((card) => {
            ...
            if (panelId) {
                ...
                if (savedExpanded && Object.prototype.hasOwnProperty.call(savedExpanded, panelId)) {
                    ...
                    expanded = defaultExpandedIds.has(panelId);
                }
    ...
    async bulkMoveBookmarksToPage(newPageId) {
        ...
        if (!Array.isArray(selectedIndexes) || selectedIndexes.length === 0) {
            this.ui.showNotification('Select one or more bookmarks first.', 'info');
            return;
        }
    ...
    refreshBookmarksFilterOptions() {
        ...
        if (bulkPageSelect) {
            ...
            this.pagesData.forEach((page) => {
                ...
                option.textContent = page.id === currentPageId ? `${page.name} (current)` : page.name;
